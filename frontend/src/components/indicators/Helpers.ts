...
export const filterByRecentWindow = (
  items: IpeaItem[],
  ms: number,
): IpeaItem[] => {
...
export const computeClosingsWithVariation = (
  items: IpeaItem[],
): ClosingRow[] => {
...
export const computeMetrics = (
  ...
  if (variations.length > 0) {
    avgDailyAbsVar6m =
      variations.reduce((s, v) => s + Math.abs(v), 0) / variations.length;
...
export const computeAggregatedTotal = (
  ...
  const variationYoY =
    hasYoY && totalYoY > 0
      ? ((totalCurrent - totalYoY) / totalYoY) * 100
      : null;
...
export const findImage = (
  ...
  const map =
    folder === "impostos"
      ...
      : folder === "exportacoes"
        ? EXPORT_IMAGES
        : folder === "cambioComercial"
          ? CAMBIO_COMERCIAL_IMAGES
          : folder === "balanca"
            ? BALANCA_IMAGES
            : INDICATOR_IMAGES;
  ...
  const match = Object.entries(map).find(([path]) => {
    ...
    const cleaned = basename
      .replace(/^balanca-pagamentos-/, "") // ← PRIMEIRO (mais específico)
      .replace(/^cambio-contratado-/, "") // ← segundo (específico)
      .replace(
        /^(exportacoes|impostos|indicadores|cambioComercial|balanca)-/,
        "",
      ); // ← por último (genérico)
...
export const describeFiveYearReturn = (r: number): string => {
  ...
  if (abs >= 80)
    return `forte ${dir} de longo prazo, com alta expressiva de ${r > 0 ? "+" : ""}${r.toFixed(2)}% no recorte de 5 anos`;
  if (abs >= 40)
    return `${dir} consistente de ${r > 0 ? "+" : ""}${r.toFixed(2)}% no recorte de 5 anos, indicando tendência estrutural positiva`;
  if (abs >= 15)
    return `${dir} moderada de ${r > 0 ? "+" : ""}${r.toFixed(2)}% no recorte de 5 anos, com trajetória ascendente acompanhada de correções`;
  if (abs >= 0.5)
    return `${dir} leve de ${r > 0 ? "+" : ""}${r.toFixed(2)}% no recorte de 5 anos, sinalizando mercado lateralizado em longo prazo`;
...
export const describeSixMonthReturn = (r: number): string => {
  ...
  if (r >= 15)
    return `fase de forte alta no curto prazo, acumulando ${s} nos últimos 6 meses`;
  if (r >= 5)
    return `fase ascendente consistente, com ${s} nos últimos 6 meses`;
  if (r >= 0.5)
    return `leve viés de alta no curto prazo, acumulando ${s} nos últimos 6 meses`;
  if (r > -0.5)
    return `mercado lateralizado nos últimos 6 meses, com variação líquida de ${s}`;
  if (r > -5)
    return `leve viés de baixa no curto prazo, acumulando ${s} nos últimos 6 meses`;
  if (r > -15)
    return `fase descendente consistente, com ${s} nos últimos 6 meses`;
...
export const describeSelicCorrelation = (
  ...
  const shortTerm =
    sixMonthReturn === null ? null : sixMonthReturn >= 0 ? "alta" : "queda";
  const longTerm =
    fiveYearReturn === null
      ? null
      : fiveYearReturn >= 0
