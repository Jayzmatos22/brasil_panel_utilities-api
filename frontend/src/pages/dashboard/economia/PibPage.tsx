...
// 1. IMAGENS
// ============================================================================
const PIB_IMAGES = import.meta.glob(
  "../../../assets/pib/*.{jpeg,jpg,png,webp,avif}",
  {
    eager: true,
    import: "default",
  },
...
function findPibImage(term: string): string | undefined {
  const match = Object.entries(PIB_IMAGES).find(([path]) =>
...
function StatePanel({
  ...
  return (
    <div
      ...
      className={`flex items-start gap-3 rounded-lg border px-4 py-3 ${toneClass}`}
    >
...
const compactBrl = (v: number) => {
  ...
  if (abs >= 1e12)
    return `R$ ${(v / 1e12).toLocaleString("pt-BR", { maximumFractionDigits: 2 })} tri`;
  if (abs >= 1e9)
...
export default function PibPage() {
  ...
  const {
    data: ipeaPibData,
    isLoading: isPibLoading,
    error: pibError,
  } = useMonthlyPib();
  const {
    data: pibEstados,
    isLoading: isEstadosLoading,
    error: estadosError,
  } = usePibPorEstado();
  ...
  const stats = useMemo(() => {
    ...
    const points = Object.entries(yearlySums)
      ...
      .filter(
        (p) => p.value > 0 && parseInt(p.date) >= 1995 && p.monthsCount === 12,
      )
  ...
  const monthlyDataForCompareYear = useMemo(() => {
    ...
    const filtered = rawData.filter(
      (item) =>
        item.data.startsWith(String(compareYear)) && item.valor !== null,
    );
  ...
  const comparisonSummary = useMemo(() => {
    ...
    const sumForYear = (y: number) => {
      return rawData
        .filter(
          (item) => item.data.startsWith(String(y)) && item.valor !== null,
        )
  ...
  const monthlyComparisonRows = useMemo(() => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const compare = monthlyDataForCompareYear.find(
        (m) => m.label === monthNames[i],
      );
  ...
  const statBlocks = useMemo(() => {
    ...
    return [
      {
        label: `Início · ${stats.first.date}`,
        value: compactBrl(stats.first.value),
        isVar: false,
      },
      {
        label: `Atual · ${stats.last.date}`,
        value: compactBrl(stats.last.value),
        isVar: false,
      },
      {
  ...
  return (
    ...
    >
      ...
      >
        {bannerImage ? (
          <>
            <div
              ...
              style={{
                background:
                  "radial-gradient(circle at 60% 40%, #009C3B 0%, #FFDF00 25%, #002776 50%, transparent 75%)",
              }}
            />
            <img
              src={bannerImage}
            ...
            <div
              ...
              style={{
                background:
                  "radial-gradient(ellipse 65% 60% at 65% 40%, transparent 0%, rgba(2,6,23,0.6) 40%, #020617 75%)",
              }}
            />
            <div className="absolute bottom-0 left-0 right-0 h-[50%] z-2 bg-linear-to-t from-[#020617] via-[#020617]/60 to-transparent" />
          </>
        ) : (
          <div className="absolute inset-0 bg-[#020617]" />
        )}

        <div className="absolute left-0 top-0 bottom-0 w-[3px] z-[3] bg-linear-to-b from-[#009C3B] via-[#FFDF00] to-[#002776] shadow-[0_0_18px_rgba(255,223,0,0.6)]" />
        ...
        <div className="relative z-10 p-6 sm:p-10 w-full">
          ...
          <p className="mt-3 max-w-2xl text-sm sm:text-base text-slate-200/50 leading-relaxed drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
            Produto Interno Bruto a preços correntes (R$). Série histórica,
            recorte anual, visão mensal e distribuição por unidade federativa.
          </p>
        </div>
      </motion.header>
      ...
      >
        ...
        >
          <div className="absolute inset-x-0 top-0 h-[2px] bg-linear-to-r from-[#009C3B] via-[#FFDF00] to-[#002776]" />
          <div
            className="absolute -top-24 -right-24 w-96 h-96 rounded-full opacity-30 blur-3xl"
            style={{
              background:
                "radial-gradient(circle, #009C3B 0%, transparent 70%)",
            }}
          />
          ...
          <div className="relative p-6 sm:p-8">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h2 className="text-sm font-medium text-white mt-1">
                    PIB anual (finalizado) mais recente
                  </h2>
                </div>
              </div>
              {currentPib && (
            ...
            {isPibLoading ? (
              ...
              <StatePanel
                ...
                description="Não foi possível obter os dados do IPEA."
              />
            ) : (
              currentPib && (
                <>
        ...
        >
          <div className="absolute inset-x-0 top-0 h-[2px] bg-linear-to-r from-[#009C3B] via-[#FFDF00]/60 to-transparent" />
          <div
          ...
          <div className="relative p-6 sm:p-8 flex flex-col gap-5">
            ...
            <div>
              <label
                htmlFor="pib-year"
                className="block text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-400 mb-1.5"
              >
                Ano principal
              </label>
            ...
            <div className="flex items-center gap-3 py-1">
              ...
              <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-500/50">
                vs
              </span>
              <div className="flex-1 h-px bg-emerald-900/30" />
            </div>
            ...
            <div>
              ...
              <div className="relative max-w-[160px]">
                ...
                <CalendarSearch
                  size={14}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-[#FFDF00]/40 pointer-events-none"
                />
              </div>
            </div>
            ...
            {pibError ? (
              <StatePanel
                tone="error"
                icon={<AlertTriangle size={16} />}
                title={`Erro ao carregar o PIB para ${year}`}
              />
            ) : pibByYear ? (
              ...
              !isPibLoading && (
                <StatePanel
                  ...
                  title={`Sem dados anuais fechados para ${year}`}
                />
              )
            )}
          </div>
          ...
          <div className="relative p-6 sm:p-8 flex flex-col gap-6">
            ...
            {isPibLoading ? (
              ...
              <StatePanel
                tone="error"
                icon={<AlertTriangle size={16} />}
                title="Erro ao carregar o detalhamento mensal"
              />
            ) : monthlyDataForYear && monthlyDataForYear.length > 0 ? (
              <>
                ...
                {comparisonSummary && (
                  ...
                  >
                      ...
                      >
                        {comparisonSummary.pct > 0
                          ...
                          : comparisonSummary.pct < 0
                            ? "Desceu "
                            : ""}
                        {Math.abs(comparisonSummary.pct).toLocaleString(
                          "pt-BR",
                    ...
                    <div className="text-right">
                      ...
                      >
                        {comparisonSummary.change > 0 ? "+" : ""}
                        {compactBrl(comparisonSummary.change)}
                      </p>
                    </div>
                  </div>
                )}
                          ...
                          return (
                            ...
                            >
                              ...
                              >
                                ...
                                {Math.abs(row.pct).toLocaleString("pt-BR", {
                                  maximumFractionDigits: 1,
                                })}
                                %
                              </span>
              ...
              <StatePanel
                tone="empty"
                icon={<Database size={16} />}
                title={`Sem dados mensais publicados para o ano de ${year}`}
              />
            )}
          </div>
        </motion.section>
        ...
        >
          {isPibLoading ? (
            ...
            <StatePanel
              tone="error"
              icon={<AlertTriangle size={16} />}
              title="Erro ao carregar a série histórica"
            />
          ) : stats ? (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {statBlocks.map((block, idx) => (
                  ...
                  >
                    ...
                    <p
                      className={`mt-1.5 text-xl font-semibold tabular-nums ${block.isVar ? block.colorClass : "text-white"}`}
                    >
                      {block.value}
                    </p>
                  </div>
            ...
            <StatePanel
              tone="empty"
              icon={<Database size={16} />}
              title="Série sem pontos suficientes para o gráfico"
            />
          )}
        </SectionCard>
      </motion.div>
        ...
        >
          {isEstadosLoading ? (
            ...
            <StatePanel
              tone="empty"
              icon={<Database size={16} />}
              title="Sem dados de PIB por estado"
            />
          )}
        </SectionCard>
      </motion.div>
