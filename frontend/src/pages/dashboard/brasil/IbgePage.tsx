...
import { Search, BarChart3, MapPin, AlertCircle, LoaderCircle } from "lucide-react";
import {
  useStates,
  useCitiesByState,
  useStatesRanking,
...
const SkeletonBlock = ({ className }: { className?: string }) => (
  <div className={`animate-pulse bg-white/5 rounded-md ${className}`} aria-hidden="true" />
);
...
const CityChip = memo(({ name }: { name: string }) => (
  <div 
    className="bg-white/[0.03] border border-white/10 rounded-lg px-4 py-2.5 text-slate-300 text-sm 
...
export default function IbgePage() {
  ...
  return (
    ...
    >
      ...
      <motion.header
        variants={item}
        className="group relative overflow-hidden rounded-2xl bg-slate-950 shadow-2xl shadow-emerald-900/20 min-h-[280px] flex items-end"
      ...
      >
        ...
        <div 
          ...
          style={{ background: 'radial-gradient(circle at 50% 40%, #009C3B 0%, #FFDF00 30%, #002776 60%, transparent 80%)' }}
        />
        ...
        <div className="absolute inset-0 z-[2] bg-gradient-to-r from-slate-950/60 via-transparent to-transparent" />
        ...
        <div className="relative z-10 p-8 sm:p-10 w-full">
          ...
          <p className="mt-3 max-w-2xl text-sm sm:text-base text-slate-300/90 leading-relaxed">
            Informações territoriais, malha municipal e dados estruturais do Brasil.
          </p>
        </div>
      </motion.header>
      ...
      >
        ...
        <div className="flex flex-col gap-1.5 flex-1 max-w-xs">
          <label htmlFor="state-select" className="text-slate-400 text-xs font-medium uppercase tracking-wider">
            Estado
          </label>
          {loadingStates ? (
            ...
            >
              <option value="" disabled className="bg-slate-900">Selecione um estado</option>
              {states?.map((s) => (
        ...
        <AnimatePresence>
          {selectedState && (
            ...
            >
              <label htmlFor="city-filter" className="text-slate-400 text-xs font-medium uppercase tracking-wider">
                Filtrar Município
              </label>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          <div className="flex items-center gap-2.5">
            <BarChart3 size={16} className="text-yellow-400" aria-hidden="true" />
          ...
          {loadingRanking ? (
            ...
            <p className="text-slate-500 text-sm py-8 text-center">Sem dados para o ranking.</p>
          )}
        </motion.div>
          ...
          {selectedState && selectedStateData ? (
            ...
            >
              <div className="flex items-center gap-3 border-b border-white/10 pb-4">
                ...
                <div>
                  <h3 className="text-white font-bold text-lg">{selectedStateData.nome}</h3>
                  <p className="text-emerald-400 text-xs font-mono font-semibold">{selectedStateData.sigla}</p>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3 text-sm">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-white font-medium">{selectedStateData.regiao.nome}</span>
                </div>
                <div className="flex justify-between items-center">
                  ...
                  {loadingCities ? (
                    ...
                    <span className="text-yellow-400 font-mono font-bold">{cities.length}</span>
                  ) : (
            ...
            <motion.div 
              variants={item}
              className="relative bg-white/[0.02] border border-dashed border-white/10 rounded-2xl p-6 flex flex-col items-center justify-center text-center gap-2"
            >
              <MapPin size={24} className="text-slate-600" />
              <p className="text-slate-500 text-sm">Selecione um estado para detalhes</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
          ...
          >
            <div className="flex items-center justify-between mb-5">
              ...
              {cities && !loadingCities && (
                <span className="text-slate-500 text-xs font-mono">{cities.length} resultados</span>
              )}
            </div>
            ...
            {loadingCities ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 max-h-[400px] overflow-y-auto scrollbar-thin pr-2">
                {cities.map((city) => (
              ...
              <div className="py-8 text-center">
                <p className="text-slate-500 text-sm">Nenhum município encontrado para este filtro.</p>
              </div>
            )}
          </motion.section>
        )}
