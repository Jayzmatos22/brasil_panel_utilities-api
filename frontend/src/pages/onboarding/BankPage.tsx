...
export default function BankPage() {
  ...
  const { data: allBanks = [], isError } = useAllBanks();

  const [bankSearch, setBankSearch]     = useState('');
  const [selectedBank, setSelectedBank] = useState<{ name: string; code: number; ispb: string } | null>(null);
  ...
  useEffect(() => {
    const raw = localStorage.getItem('currentUser');
    if (!raw) navigate('/login-usuario', { replace: true });
  }, [navigate]);

  useEffect(() => {
    if (isError) toast.error('Erro ao carregar bancos');
  }, [isError]);
  ...
  const handleSelectBank = (bank: { name: string; code: number; ispb: string }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const raw = localStorage.getItem('currentUser');
    if (!raw) { toast.error('Sessão expirada'); navigate('/login-usuario'); return; }
  ...
  const inputClass =
    'w-full h-12 p-3 rounded-md bg-slate-950 text-white placeholder-slate-500 border border-slate-600 outline-none focus:ring-2 focus:ring-yellow-500 transition-all';
    ...
    <main className="min-h-screen flex items-center justify-center bg-slate-950 p-4">
      ...
      >
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="relative flex flex-col gap-1">
              ...
              {showDropdown && bankSearch && !selectedBank && filteredBanks.length > 0 && (
                <ul className="absolute top-full mt-1 w-full bg-slate-800 border border-slate-600 rounded-lg max-h-48 overflow-y-auto z-50 shadow-xl">
        ...
        <div className="border-t border-slate-700 pt-4">
          <button
            type="submit"
            className="w-full h-12 bg-yellow-500 hover:bg-yellow-600 text-white font-bold rounded-md transition-all cursor-pointer flex items-center justify-center gap-2 active:scale-95"
          >
            <Save size={18} />
