...
import { User, Trash2, ShieldAlert, KeyRound, AlertOctagon } from 'lucide-react';
import { FormField } from '../../components/forms/FormField';
import { SubmitButton } from '../../components/forms/SubmitButton';
import { container, item } from '../../lib/motion/presets';
import { useUpdateName, useUpdatePassword, useDeleteAccount } from '../../hooks/UseSettings';
...
const themeMap = {
  green: {
    ...
    topLine: 'bg-gradient-to-r from-emerald-500/50 via-green-400/30 to-transparent',
  ...
  yellow: {
    ...
    topLine: 'bg-gradient-to-r from-amber-500/50 via-yellow-400/30 to-transparent',
  ...
  red: {
    ...
    bg: 'bg-red-500/[0.04]',
    topLine: 'bg-gradient-to-r from-red-600/60 via-red-400/30 to-transparent',
...
function SettingsSection({
  icon, title, description, children, theme = 'green',
}: {
  ...
  return (
    ...
    >
      ...
      <div className={`absolute top-0 left-0 right-0 h-[2px] ${styles.topLine}`} />
      
      <div className="flex items-center gap-4 pb-5 border-b border-slate-800/60">
        <span className={`flex h-10 w-10 items-center justify-center rounded-xl ${styles.iconBg} ${styles.iconColor} shrink-0 transition-colors`}>
          {icon}
        </span>
        <div>
          <h2 className={`font-semibold text-base tracking-tight ${styles.titleColor}`}>
            {title}
          </h2>
...
export default function SettingsAuthPage() {
  ...
  const { mutate: changeName,     isPending: pendingName     } = useUpdateName(() => setName(''));
  const { mutate: changePassword, isPending: pendingPassword } = useUpdatePassword(() => {
    setCurrentPassword('');
    setNewPassword('');
    setConfirmPassword('');
  });
  const { mutate: deleteAccount,  isPending: pendingDelete   } = useDeleteAccount();

  const handleChangeName = (e: FormEvent) => {
    e.preventDefault();
    if (!name.trim())                          { toast.error('Digite o novo nome.');           return; }
    if (name.trim().split(' ').length < 2)     { toast.error('Digite nome e sobrenome.');      return; }
    ...
    >
      ...
      <motion.div variants={item} className="relative z-10">
        <div className="flex items-center gap-3 mb-2">
          ...
          <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-600">Preferências</span>
        ...
        <p className="text-slate-500 text-sm mt-2">Gerencie suas informações pessoais e a segurança da sua conta.</p>
      </motion.div>
      ...
      >
        {quickNavItems.map((navItem) => (
          ...
          >
            <span className={`flex h-8 w-8 items-center justify-center rounded-lg bg-slate-800/80 group-hover:bg-slate-700/50 ${navItem.iconColor} transition-colors shrink-0`}>
              {navItem.icon}
            </span>
          ...
          <form onSubmit={handleChangeName} className="flex flex-col gap-5">
            ...
            <div className="flex justify-end">
              <SubmitButton
                ...
                className="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 text-white shadow-[0_0_20px_rgba(16,185,129,0.15)] hover:shadow-[0_0_25px_rgba(16,185,129,0.3)] transition-all duration-300 px-8"
              />
            </div>
          </form>
        </SettingsSection>
          ...
          <form onSubmit={handleChangePassword} className="flex flex-col gap-5">
            ...
            <div className="flex justify-end">
              <SubmitButton
                ...
                className="bg-gradient-to-r from-amber-600 to-yellow-500 hover:from-amber-500 hover:to-yellow-400 text-slate-950 font-semibold shadow-[0_0_20px_rgba(245,158,11,0.15)] hover:shadow-[0_0_25px_rgba(245,158,11,0.3)] transition-all duration-300 px-8"
              />
            </div>
          </form>
        </SettingsSection>
          ...
          <form onSubmit={handleDeleteAccount} className="flex flex-col gap-5">
            <div className="flex items-start gap-4 bg-red-500/[0.07] border border-red-500/20 rounded-xl p-5">
              ...
              <div className="space-y-1">
                ...
                <p className="text-red-300/70 text-xs leading-relaxed">
                  Ao deletar sua conta, todos os seus dados, histórico e acessos serão permanentemente removidos dos nossos servidores. Esta ação <strong>não pode ser desfeita</strong> e não há como recuperar os dados posteriormente.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4 pt-2">
              ...
              <button
                ...
                className="w-full h-12 bg-gradient-to-r from-red-700 to-rose-600 hover:from-red-600 hover:to-rose-500 text-white font-bold
              ...
              >
                <Trash2 size={16} />
                {pendingDelete ? 'Deletando conta…' : 'Deletar permanentemente minha conta'}
              </button>
            </div>
          </form>
        </SettingsSection>
